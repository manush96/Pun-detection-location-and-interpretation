...
def score_function(new_trigram ,old_trigram ,old_word, new_word):
	...
	score = (trigram_freq_dict[old_trigram] - trigram_freq_dict[new_trigram]) - (1/(ratio ** 10))
...
def detect(query,count,output_dict):
	...
	for trigram in trigrams:
		...
		for unigram in unigrams:
			...
			new_keyset = rhyme(unigram,2)

			for new_word in new_keyset:
				POS_Tag_Set =("NN","NNS","JJ","JJR","JJS","RBR","RB","RBS","VB","VBD","VBG","VBN","VBP","VBZ","CD")
				tag_new_word = nltk.pos_tag(new_word)
				new_trigram = trigram.replace(unigram , new_word)
	...
	if len(score_pair_dict.keys()) > 0:
			pair = dict(sorted(score_pair_dict.items(), key=operator.itemgetter(1), reverse=True)[:20])
			output_dict.update(pair)
			print(query)
			print(pair)
