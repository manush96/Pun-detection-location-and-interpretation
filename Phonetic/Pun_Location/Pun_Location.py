...
def leven_distance(old_word, new_word):
	...
	w_len_ch = max(w_len_ch_old ,w_len_ch_old)
	...
	if(len(pp.phones_for_word(old_word)))>0 and len(pp.phones_for_word(new_word))>0:
		...
		w_len_phs = max(w_len_phs_old,w_len_phs_new)
		...
		w_len_ph = max(w_len_ph_old,w_len_ph_new)

		#distance
		dis_ph  = Levenshtein.distance(old_word_ph, new_word_ph)
		ratio_ph =  (w_len_ph - dis_ph)/w_len_ph

	#Assigning a large negative value to get only true cases
	else:
		ratio_ph  = -1000
		ratio_phs = -1000
...
def score_function(new_trigram ,old_trigram ,old_word, new_word):
	...
	if ratio > 0:
		score = (trigram_freq_dict[new_trigram] - trigram_freq_dict[old_trigram]) - (1/(ratio ** 10))
	else:
		score = -1000
...
def score_pair(new_trigram ,old_trigram ,old_word, new_word, score_pair_dict):
	score = score_function(new_trigram ,old_trigram ,old_word, new_word)
	if score > 0:
...
def detect(query,count,output_dict):
	...
	for trigram in trigrams:
		...
		#Getting the frequency of ngrams
		main(trigram, fdict)

		#Considering only less frequent ngrams
		...
		#For each unigram generating candidate word
					...
					if len(replace_dict.keys())>0:
							...
							for new_trigram in replace_dict.keys():
								...
								if len(diffset) > 0:
											...
											#Getting finally the candidate word
	...
	#Checking finally we got result or not
	if len(score_pair_dict.keys()) > 0:

			#Getting the pair of word with highest score
			pair = max(score_pair_dict, key=score_pair_dict.get)

			#Storing result in output dictionary
			output_dict.update({count : pair.split()[0]})
			print(query)
			print(pair)

	#Fallback method choosing last word
	else:
		output_dict.update({count : query.split()[-1]})
		print(query)
		print(query.split()[-1])
...
for file in glob.glob("*"):
	...
	with open(file,"r",encoding="ISO-8859-1") as f:
		...
		#Detecting location of pun words
		for line in lines:
				count = count + 1
